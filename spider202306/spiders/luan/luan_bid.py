...
# 工程建设，不限，不限，开评标信息，不限，公开招标
payload = "{\"token\":\"\",\"pn\":0,\"rn\":10,\"sdt\":\"\",\"edt\":\"\",\"wd\":\"\",\"inc_wd\":\"\",\"exc_wd\":\"\",\"fields\":\"title\",\"cnum\":\"001\",\"sort\":\"{'webdate':'0'}\",\"ssort\":\"title\",\"cl\":500,\"terminal\":\"\",\"condition\":[{\"fieldName\":\"categorynum\",\"isLike\":true,\"likeType\":2,\"equal\":\"001001005\"}],\"time\":[{\"fieldName\":\"webdate\",\"startTime\":\"1970-01-01 00:00:00\",\"endTime\":\"2999-12-31 23:59:59\"}],\"highlights\":\"\",\"statistics\":null,\"unionCondition\":[],\"accuracy\":\"\",\"noParticiple\":\"0\",\"searchRange\":null,\"isBusiness\":\"1\"}\r\n"
...
for i in range(50):
    ...
    for project in projects['result']['records']:
        ...
        if '中标结果公示' in title:
            ...
            kzj, bid_amount = get_bid_mess(html_text)
        print(kzj, bid_amount)
        data = pd.DataFrame(data=[[id, title, relationguid, link_url, kzj, bid_amount, marker]],
                            columns=['id', '项目名称', '关系id', '中标公告网址', '控制价', '中标金额', '数据标记'])
        df = pd.concat([df, data])
    sleep(random.randint(1, 5))
df.to_excel(f'六安市中标公告信息{t}.xlsx', index=False)
